...
<p align="center">名片点赞、戳一戳回图、群头衔与刷屏管理互动插件。</p>
...
<p align="center">
  ...
  <img alt="版本" src="https://img.shields.io/github/v/tag/TogetsuDo/pallas-community-plugin-interact?label=%E7%89%88%E6%9C%AC&color=2563EB">
</p>
...
- 开启刷屏管理后：同一成员短时间连续发言达到阈值时，随机禁言该普通成员。
...
刷屏管理不是命令权限，是否执行由插件配置和牛牛在群内的管理员身份共同决定。
...
| `enable_spam_moderation` | 是否启用刷屏管理，默认关闭 |
| `spam_message_threshold` | 时间窗内触发禁言所需的消息数，默认 4 |
| `spam_window_sec` | 刷屏判定时间窗，默认 5 秒 |
| `spam_mute_min_sec` / `spam_mute_max_sec` | 随机禁言时长上下限，默认 10~60 秒 |
...
| 刷屏成员没有被禁言 | 确认已开启 `enable_spam_moderation`，牛牛是群管理员或群主，且目标不是群管/群主。 |
...
- [`handlers.py`](./handlers.py)：实现点赞、戳一戳回图、群头衔设置与刷屏管理核心行为。
- [`notices.py`](./notices.py)：挂载戳一戳通知、刷屏管理与 `/群头衔` 消息规则。
- [`service.py`](./service.py)：负责点赞调用、图片目录解析和图片候选收集。
- [`spam.py`](./spam.py)：负责刷屏时间窗内的消息阈值判断。
...
- 刷屏管理默认关闭；开启后仅对普通成员生效，管理员和群主不会被自动禁言。
